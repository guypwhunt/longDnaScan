...
mkdir -p Output/QualityControl/FastQCTwo
...
fastqc "${outputDirectory}"Output/ReadTrimmingAndFiltering/NanoFilt/NanoFilt.fastq --outdir "${outputDirectory}"Output/QualityControl/FastQCTwo -t $threads

echo ""
echo "fastqc2 complete"
echo ""
...
bwa mem -t $threads "${outputDirectory}"Output/ErrorCorrection/Racon/miniasm.racon.consensus.fasta -x ont2d "${outputDirectory}"Output/ReadTrimmingAndFiltering/NanoFilt/NanoFilt.fastq > "${outputDirectory}"Output/ErrorCorrection/Pilon/miniasm_racon.sam
